...
static const uint8_t *TxBufPtr;              //буфер передачи
static uint8_t TxBufSize;                    //размер буфера передачи
static USART::TProc   TxProc;                //протокол передачи
static uint8_t TxCrc;                        //контрольная сумма процесса передачи 
static dks_tx_proc::TTxPhase TxPhase;        //фаза процесса передачи
...
namespace USART
{
  ...
  constexpr uint32_t MAX_DEDT    = 31U;
  constexpr uint32_t MAX_DEAT    = 31U;
...
void TUSART::pin_clk_config()
{
  ...
  if (!LL_AHB2_GRP1_IsEnabledClock(USARTHw.PinClkMask))    //включение тактирования GPIO для порта с USART, если не включено
  {
    LL_AHB2_GRP1_EnableClock(USARTHw.PinClkMask);
  }
  if (                                                     //включение тактирования от шины APB2 для USART1!, если не включено
      (USARTHw.USARTx == USART2) 
      && 
      !LL_APB1_GRP1_IsEnabledClock(USARTHw.PeriphClkMask)
     )
  {
...
void TUSART::usart_hw_init()
{
  ...
  LL_USART_Init(USARTHw.USARTx, &USART_InitStruct);
...
void TUSART::sens_dma_set()
{
  ...
  */
  ...
  LL_DMA_SetPeriphRequest(DmaNum, LL_DMA_CHANNEL_6, LL_DMA_REQUEST_2);
  LL_DMA_SetPeriphRequest(DmaNum, LL_DMA_CHANNEL_7, LL_DMA_REQUEST_2);
...
{	 
  ...
  */
  ...
  constexpr uint32_t F_CNT         = 36000U; //частота работы счетчика таймера
...
void TUSART::tx_burst(const uint8_t *DataBurst, uint16_t Size, TProc Proc)
{
  uint8_t FirstByte;
